...
#define SQR(x)                  ((x)*(x)*1LL)
//#define RANGE(x1, y1, x2, y2)   (sqrt(SQR(x1-x2) + SQR(y1-y2)))

long double RANGE(long double x1, long double y1, long double x2, long double y2)
{
...
struct point
{
    ...
    long double     y;
    point(){}
    point(int ind, long double rast, long double x, long double y) : ind(ind), rast(rast), x(x), y(y) {}
};
